...
%------------各旬、年日最低气温≤0℃日数、初终期（nyyzz=10280）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 10280
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a各旬、年日最低气温≤0℃日数、初终期（nyyzz=10280）.txt','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('C:\Users\ZhanLF\Desktop\b各旬、年日最低气温≤0℃日数、初终期（nyyzz=10280）.txt','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
