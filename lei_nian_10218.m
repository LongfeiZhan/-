...
%批量读取文件
file1 = dir('F:\JXClimatePic\统计资料\Years');
filedata = [];
for n = 3 : length(file1)
    temp = dir(['F:\JXClimatePic\统计资料\Years\',file1(n).name]);
...
%------------日平均气温稳定通过各级界限温度有关统计值（nyyzz=10218）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a日平均气温稳定通过各级界限温度有关统计值（nyyzz=10218）.xls','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('C:\Users\ZhanLF\Desktop\b日平均气温稳定通过各级界限温度有关统计值（nyyzz=10218）.xls','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
