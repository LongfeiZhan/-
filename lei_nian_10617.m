...
%------------各月、年雨淞/雾淞和累年各月最多、少日数（nyyzz=10617）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 10617
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a各月、年雨淞和累年各月最多、少日数（nyyzz=10617）.xls','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('C:\Users\ZhanLF\Desktop\b各月、年雨淞和累年各月最多、少日数（nyyzz=10617）.xls','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
