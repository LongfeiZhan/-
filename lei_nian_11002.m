...
%----------3、各月、年最大/极大/最大定时风速及风向、出现日期（nyyzz=11002/11003/21001）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 11002
...
%-------写出到txt-------
fid = fopen('F:\江西省气候图集\解码结果\a各月、年最大、极大、最大定时风速及风向、出现日期（nyyzz=11002）.txt','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('F:\江西省气候图集\解码结果\b各月、年最大、极大、最大定时风速及风向、出现日期（nyyzz=11002）.txt','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
