...
%批量读取文件
file1 = dir('F:\JXClimatePic\结果\Years');
filedata = [];
for n = 3 : length(file1)
    temp = dir(['F:\JXClimatePic\结果\Years\',file1(n).name]);
...
%------------各旬、年日最高气温 ≥ 35℃日数、初终期(nyyzz=10278)-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 10278
        ...
        rr = 1;
        for j = 1 : (mm-3)*88 %88指的是软件中累年的一行数据存在b中需要多少行
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a累年各旬、年日最高气温 ≥ 35℃日数、初终期(nyyzz=10278).xls','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('C:\Users\ZhanLF\Desktop\b累年各旬、年日最高气温 ≥ 35℃日数、初终期(nyyzz=10278).xls','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
