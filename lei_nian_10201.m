...
%------------各月及年平均气温（nyyzz=10201）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 10201
...
%-------写出到txt-------
fid = fopen('F:\江西省气候图集\解码结果\a各月及年平均气温（nyyzz=10201）.txt','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('F:\江西省气候图集\解码结果\b各月及年平均气温（nyyzz=10201）.txt','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
