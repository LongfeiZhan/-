...
%------最热月14时平均相对湿度（nyyzz=20301）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 20301
...
%-------写出到txt-------
fid = fopen('F:\江西省气候图集\解码结果\a最热月14时平均相对湿度（nyyzz=20301）.txt','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('F:\江西省气候图集\解码结果\b最热月14时平均相对湿度（nyyzz=20301）.txt','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
