...
%------------无霜期和累年最长、最短无霜期（nyyzz=20706）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 20706
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a无霜期和累年最长、最短无霜期（nyyzz=20706）.xls','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('C:\Users\ZhanLF\Desktop\b无霜期和累年最长、最短无霜期（nyyzz=20706）.xls','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
