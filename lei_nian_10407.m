...
%------------总云量<2成日数(3次)（nyyzz=10407）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 10408
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a总云量<2成日数（nyyzz=10407）.txt','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('F:\JXClimatePic\解码结果\b总云量<2成日数（nyyzz=10407）.txt','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
