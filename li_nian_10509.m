...
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 10509
        ...
        rr = 1;
        for j = 1 : (mm-3)*27  %此处'27'根据具体情况调整数值（变量b中年段之间多少行就改成多少数值）
...
%-------写出到txt-------
fid = fopen('C:\Users\ZhanLF\Desktop\a10509.xls','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('C:\Users\ZhanLF\Desktop\b10509.xls','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
