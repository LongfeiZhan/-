...
%------2、	各月、年最低地面温度（nyyzz=11103）-----------
c = 1; c3 = 1;
for i = 1 : length(filedata)
    if str2num(filedata(i).name(2:6)) == 11103
...
%-------写出到txt-------
fid = fopen('F:\JXClimatePic\解码结果\a各月、年最低地温.txt','w');
[mmm,nnn]=size(a);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
...
fid = fopen('F:\JXClimatePic\解码结果\b各月、年最低地温.txt','w');
[mmm,nnn]=size(b);
for i=1:1:mmm
    for j=1:1:nnn
        if j==nnn
